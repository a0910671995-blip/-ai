...
// 4. 視圖：市場總覽 (Home)
// ==========================================
function TwStocksDashboard({ twStocks, twUpdateTime, loading, error }) {
  const [searchTerm, setSearchTerm] = useState('');

  const filtered = useMemo(() => {
    let list = Array.isArray(twStocks) ? [...twStocks] : [];
    const s = String(searchTerm || '').toUpperCase();
    if (!s) return list.slice(0, 100); // 預設顯示前100熱門股
    return list.filter(t => String(t.symbol || '').includes(s) || String(t.name || '').includes(s)).slice(0, 200);
  }, [twStocks, searchTerm]);

  if (loading && (!twStocks || !twStocks.length)) {
    return <div className="text-center py-32 text-slate-500"><RefreshCw className="w-8 h-8 animate-spin mx-auto mb-4" /> 抓取全台股資料中...</div>;
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {filtered.map(stock => (
          <TwLiveStockCard key={stock.symbol} stock={stock} isScanned={false} />
        ))}
...
function PortfolioPage({ twLivePrices }) {
  ...
  const activeSymbols = [...new Set(portfolio.map(p => p.symbol))];
    ...
    <div className="animate-in fade-in duration-300">
      ...
      {portfolio.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {portfolio.map(p => {
            const livePrice = twLivePrices[p.symbol] || p.avgCost; // 如果沒抓到現價，預設為成本價
  ...
  return (
    <div className="min-h-screen bg-[#0b0e14] text-slate-100 font-sans pb-10">
      ...
      <header className="bg-[#121620]/95 backdrop-blur border-b border-[#2a2f3a] sticky top-0 z-20 h-16 shadow-xl flex items-center px-4 justify-between">
        ...
        <div className="bg-[#1a1e27] px-4 py-2 rounded-xl border border-[#2a2f3a] text-sm font-mono text-white font-bold">波段專屬版本</div>
      </header>
